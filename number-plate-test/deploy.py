...
all_plate_info = pd.DataFrame(columns=['frame_number', 'plate_num', 'coords'])
...
def plot_boxes(results, frame, classes):
    ...
    for i in range(n):
        row = cord[i]
        if row[4] >= 0.30:  ### threshold value for detection. We are discarding everything below this value
    ...
    # Convert the plate_info list to a DataFrame
    df = pd.DataFrame(plate_info)

    # Append the DataFrame for the current frame to the global DataFrame containing information for all frames
    all_plate_info = pd.concat([all_plate_info, df], ignore_index=True)
    # Save the DataFrame containing information for all frames to a CSV file
    all_plate_info.to_csv('all_plate_info.csv', index=False)
...
def main(img_path=None, vid_path=None, vid_out=None):
    ...
    # model = torch.hub.load('', 'custom', source='local', path='last.pt',
    ...
    if img_path != None:
        ...
        img_out_name = f"./result_{img_path.split('/')[-1]}"
...
main(vid_path="proto1.mp4",vid_out="vid_1.mp4") ### for custom video
# main(vid_path=0,vid_out="webcam_facemask_result.mp4") #### for webcam

# main(img_path="wrong1.jpg")  ## for image